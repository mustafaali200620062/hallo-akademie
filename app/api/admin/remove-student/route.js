...
export async function DELETE(request) {
  try {
    ...
    await db.delete(joinRequests).where(eq(joinRequests.student_id, id))

    // 2. حذف محاولات الاختبارات
    await db.delete(examAttempts).where(eq(examAttempts.student_id, id))

    // 3. حذف نقاط الطالب
    await db.delete(studentPoints).where(eq(studentPoints.student_id, id))

    // 4. حذف ربط الطالب بالمجموعات
    await db.delete(groupStudents).where(eq(groupStudents.student_id, id))
